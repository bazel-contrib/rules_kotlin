...
## Installation
Add the Bzlmod snippet from the [release you wish to use](https://github.com/bazel-contrib/rules_kotlin/releases)
to your `MODULE.bazel` file. On Bazel 7, also set `common --enable_bzlmod` in `.bazelrc`.

Releases contain sources: Bazel compiles the workers and compiler plugins as part of your build,
just as it does with a development override. New source-only releases require Bzlmod. Legacy
WORKSPACE users must migrate to Bzlmod or continue using a release with precompiled tools.
...
and then in your `MODULE.bazel` file register it:
...
To choose a different `kotlinc` distribution, configure the module extension:
